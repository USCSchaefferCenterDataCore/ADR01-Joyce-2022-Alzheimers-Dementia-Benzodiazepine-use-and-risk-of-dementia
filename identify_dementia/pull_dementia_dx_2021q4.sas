...
%let prevminyear=1999;
%let prevmaxyear=2021;

%let clmbyear=1999; * Doing from 2017 just in case I used quarterly from 2017 on;
%let clmeyear=2021;
...
%macro getdx(ctyp,byear,eyear,dxv=,dropv=,keepv=,byvar=);
	%do year=&byear %to &eyear;
		data &tempwork..dementia_dx_&ctyp._&year.;
...
proc sort data=&tempwork..dementia_dx_&ctyp._&year; by bene_id year demdx_dt clm_typ; run;
%end;
%end;
%mend getdx;
...
data &tempwork..dementia_dx_&ctyp._&minyear._&maxyear;
		set 
	%do year=&clmbyear %to 2021;
		&tempwork..dementia_dx_&ctyp._&year.
	%end; ;
...
%do year=&byear %to &eyear;
data &tempwork..dementia_dx_carline_&year.;
		length clm_thru_dt 8.;
		set 
			%if &year<&rifqyear %then %do;
				%do mo=1 %to 12;
       ...
       otherwise line_dxtype="X"; 
		...
		label line_ccwdem="Whether carrier line dx =CCW dementia dx"
      		...
      		;
run;
data &tempwork..dementia_dx_carmrg_&year;
		merge &tempwork..dementia_dx_bcarrier_&year (in=_inclm drop=year)
					&tempwork..dementia_dx_carline_&year  (in=_inline drop=clm_typ prf_physn_npi prf_physn_upin prvdr_spclty rename=(demdx_dt=linedx_dt));
		by bene_id clm_id;
...
run;
proc sort data=&tempwork..dementia_dx_carmrg_&year.; by bene_id year demdx_dt clm_typ; run;
%end;
%mend;
...
%macro appendcarline;
%do year=&clmbyear %to 2021;
proc sort data=&tempwork..dementia_dx_carline_&year.
out=&tempwork..dementia_dx_carline_&year.;
by bene_id year demdx_dt clm_typ;
run;
%end;

data &tempwork..dementia_dx_carline_&minyear._&maxyear;
		set 
	%do year=&clmbyear %to 2021;
...
proc contents data=&tempwork..dementia_dx_carmrg_&minyear._&maxyear; run;
