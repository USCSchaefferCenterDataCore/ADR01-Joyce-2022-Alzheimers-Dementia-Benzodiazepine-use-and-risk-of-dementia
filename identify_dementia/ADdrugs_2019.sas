	...
	%do mo=1 %to 12;
		%if &mo<10 %then %do;
			proc sql;
				...
				from pde&year..pde_demo_&year._0&mo as x inner join demdx.ad_ndc&maxyear. as y
		...
		%else %do;
			proc sql;
				...
				select x.bene_id, x.srvc_dt, x.pde_id, x.days_suply_num as dayssply, &year as year, 
				(y.ndc ne "") as Addrug, y.donep, y.galan, y.meman, y.rivas, y.ndc, y.bn, y.gnn
				from pde&year..pde_demo_&year._&mo as x inner join demdx.ad_ndc&maxyear. as y
				on x.prod_srvc_id=y.ndc
				order by bene_id, year, srvc_dt;
			quit;
		%end;
