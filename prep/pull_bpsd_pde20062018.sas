...
%do year=&minyear %to &maxyear;
	%do mo=1 %to 12;
		%if &mo<10 %then %do;
			proc sql;
				create table &tempwork..proj_&year._&mo as
				select x.bene_id, x.srvc_dt as srvc_dt format=mmddyy10., x.pde_id, x.days_suply_num as dayssply, &year as year, 
				y.*
				from pde&year..pde_demo_&year._0&mo as x inner join psy.antipsych_ndclist_multum as y
		...
		%else %do;
			proc sql;
				...
				select x.bene_id, x.srvc_dt as srvc_dt format=mmddyy10., x.pde_id, x.days_suply_num as dayssply, &year as year, 
				y.*
				from pde&year..pde_demo_&year._&mo as x inner join psy.antipsych_ndclist_multum as y
				on x.prod_srvc_id=y.ndc
				order by bene_id, year, srvc_dt;
			quit;
		%end;
...
%macro setall;
data psy.bpsd_pde0618;
	format year best4. srvc_dt mmddyy10.;
	set %do yr=&minyear. %to &maxyear.;
		&tempwork..proj_&yr._1-&tempwork..proj_&yr._12
	%end;;
