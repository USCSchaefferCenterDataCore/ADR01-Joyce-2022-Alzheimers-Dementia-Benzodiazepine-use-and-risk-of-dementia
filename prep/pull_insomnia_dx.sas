...
***** insomnia Codes;
%let icd9="30742" "78052"; 
%let icd10="F5101" "G4700";

%let primary="30742" "F5101";
%let unspec="G4700" "78052";

%macro getdx(ctyp,byear,eyear,dxv=,dropv=,keepv=,byvar=);
	%do year=&byear %to &eyear;
		data &tempwork..insomniadx_&ctyp._&year;
		...
		length insomniadx1-insomniadx&maxdx $ 5;
		...
		* Keep first 5 dx codes found;
		...
		array insomniadx [*] insomniadx1-insomniadx&maxdx;
		...
		do i=1 to dim(diag);
			if diag[i] in(&icd9,&icd10) then ndx=ndx+1; * Counting total number of insomnia diagnoses;
			if diag[i] in (&icd9,&icd10) then do; 
				found=0;
				do j=1 to dxsub;
					if diag[i]=insomniadx[j] then found=j;
				end;
				if found=0 then do;
					dxsub=dxsub+1;
					if dxsub<=&maxdx then insomniadx[dxsub]=diag[i];
				end;
			end;
		end;
		
		if ndx=0 then delete;
		else insomniadx_dt=clm_thru_dt;
...
proc sort data=&tempwork..insomniadx_&ctyp._&year; by bene_id year insomniadx_dt clm_typ; run;
%end;
%end;
%mend getdx;
...
data &tempwork..insomniadx_&ctyp._&minyear._&maxyear;
		set 
	%do year=&minyear %to &maxyear;
		&tempwork..insomniadx_&ctyp._&year
	%end; ;
	by bene_id year insomniadx_dt clm_typ;
run;

%mend;

%getdx(bcarrier,&minyear,2004,dxv=prncpal_dgns_cd,dropv=,keepv=clm_id,byvar=clm_id);
...
%do year=&byear %to &eyear;
data &tempwork..insomniadx_carline_&year;
		set 
			%if &year<2017 %then %do;
				%do mo=1 %to 12;
					%if &mo<10 %then %do;
		...
		insomniadx_dt=clm_thru_dt;
		clm_typ="6";
      
		drop clm_thru_dt;
run;
data &tempwork..insomniadx_carmrg_&year;
		merge &tempwork..insomniadx_bcarrier_&year (in=_inclm drop=year)
			  &tempwork..insomniadx_carline_&year  (in=_inline rename=(insomniadx_dt=linedx_dt));
		by bene_id clm_id;

		infrom=10*_inclm+_inline;
		
		length n_found n_added matchdt _maxdx in_line 3;
		length _insomniadx1-_insomniadx&maxdx $ 5;
		retain n_found n_added matchdt _maxdx in_line _insomniadx1-_insomniadx&maxdx _insomniadx_dt;
		
		array insomniadx [*] insomniadx1-insomniadx&maxdx;
		array _insomniadx [*] _insomniadx1-_insomniadx&maxdx;
		...
		if first.clm_id then do;
				...
				in_line=0;
				do i=1 to dim(insomniadx);
					_insomniadx[i]=insomniadx[i];
				end;
				if _inclm then _insomniadx_dt=insomniadx_dt;
				else _insomniadx_dt=linedx_dt;
		end;
		...
		if _inline then do; * if in line file then keeping track of new diagnoses;
			line_found=0;
			do i=1 to _maxdx;
				if line_icd_dgns_cd=_insomniadx[i] then line_found=1;
			end;
			if line_found=1 then do; * keep track of codes found on base file;
				n_found=n_found+1;
				matchdt=matchdt+(linedx_dt=insomniadx_dt);
			end;
			else do; * add unfound code;
				_maxdx=_maxdx+1;
				if 0<_maxdx<=&maxdx then _insomniadx[_maxdx]=line_icd_dgns_cd;
				n_added=n_added+1;
				if infrom=11 then matchdt=matchdt+(linedx_dt=insomniadx_dt);
				else if infrom=1 then _insomniadx_dt=linedx_dt;
			end;	
	...
	if last.clm_id then do;
			dx_max=_maxdx;
			do i=1 to dim(insomniadx);
				insomniadx[i]=_insomniadx[i];
			end;
			insomniadx_dt=_insomniadx_dt;
			year=year(insomniadx_dt);
			output;
	end;
	
	drop line_icd_dgns_cd line_dxtype _maxdx i _insomniadx:;
	format linedx_dt insomniadx_dt mmddyy10.;
	
run;
proc sort data=&tempwork..insomniadx_carmrg_&year; by bene_id year insomniadx_dt clm_typ; run;
%end;
%mend;
...
%macro combine_dts(typ,dropv=);
		...
		title2 insomniadx_&typ._&minyear._&maxyear to dementia_dxdt_&typ._&minyear._&maxyear;

		data &tempwork..insomniadt_&typ._&minyear._&maxyear.;
				format bene_id year insomniadx_dt;
				set &tempwork..insomniadx_&typ._&minyear._&maxyear (drop=&dropv where=(not missing(bene_id)));
				by bene_id year insomniadx_dt;
				
				length _insomniadx1-_insomniadx&maxdx $ 5;
				length n_claim_typ  _dxmax _dxmax1 3;
				retain n_claim_typ _dxmax _dxmax1 _insomniadx1-_insomniadx&maxdx;
				
				array insomniadx [*] $ insomniadx1-insomniadx&maxdx;
				array _insomniadx [*] $ _insomniadx1-_insomniadx&maxdx;
				
				* First claim on this date. Save dementia dx-s into master list;
				if first.insomniadx_dt=1 then do;
						do i=1 to dim(insomniadx);
							_insomniadx[i]=insomniadx[i];
						end;
						_dxmax=dx_max;
				...
				* subsequent claim on same date. Add any dementia dx not found in first claim;
						...
						do i=1 to dx_max;
							dxfound=0;
							do j=1 to _dxmax;
								if insomniadx[i]=_insomniadx[j] then dxfound=1;
							end;
							if dxfound=0 then do; * new dx, update list;
								_dxmax=_dxmax+1;
								if _dxmax<&maxdx then _insomniadx[_dxmax]=insomniadx[i];
      	...
      	* output one obs per service_type and date;
      	if last.insomniadx_dt=1 then do;
            ...
            ;
        drop _insomniadx: _dxmax i j dxfound _dxmax1;
    run;
%mend combine_dts;
...
data &outlib..insomnia_dxdt_&minyear._&maxyear.;
		
		merge &tempwork..insomniadt_inpatient_&minyear._&maxyear 
			  &tempwork..insomniadt_outpatient_&minyear._&maxyear
			  &tempwork..insomniadt_snf_&minyear._&maxyear 
			  &tempwork..insomniadt_hha_&minyear._&maxyear 
			  &tempwork..insomniadt_carmrg_&minyear._&maxyear;
		by bene_id year insomniadx_dt clm_typ;
				
		mult_clmtyp=(first.clm_typ=0 or last.clm_typ=0);
		
		* Output summarized events, across claim types on same date?;
		length claim_types $ 5  _insomniadx1-_insomniadx&maxdx $ 5;
		length n_claims n_add_dxdate _dxmax _dxmax1 dxfound 3;
		retain claim_types n_claims _dxmax _dxmax1 _insomniadx1-_insomniadx&maxdx;
		...
		if last.insomniadx_dt=1 then do;
				* restore original variables with updated ones;
				...
				do i=1 to dim(insomniadx);
					if insomniadx[i] in(&primary) then primary=1;
					if insomniadx[i] in(&unspec) then unspec=1;
				end;

				output;
		end;
		
   	drop _insomniadx: _dxmax _dxmax1 i j dxfound clm_typ n_claim_typ n_add_typ;
